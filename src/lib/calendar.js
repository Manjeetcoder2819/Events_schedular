...
export async function POST(request) {
  try {
    ...
    // Minimal validation
    if (!eventData.summary || (!eventData.start?.dateTime && !eventData.start?.date) || (!eventData.end?.dateTime && !eventData.end?.date)) {
    ...
    const { error } = await supabase.from('event').insert([{
      ...
      user_email: session.user.email, // link to user
    }])
